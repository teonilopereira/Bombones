...
namespace Bombones.Windows.Helpers
{
    public static class CombosHelper
    {
        ...
        public static void CargarComboFormasDeVentas(ref ComboBox cbo,
     IServiceProvider? serviceProvider)
        {
            _serviceProvider = serviceProvider;
            IServiciosFormasDeVenta? servicio = _serviceProvider?.GetService<IServiciosFormasDeVenta>();
            var listaFormasDeVenta = servicio?.GetLista(null, null);
            var defaultFormasDeVenta = new FormasDeVenta()
            {
                FormasDeVentaid = 0,
                formasDeVenta = "Seleccione"
            };
            listaFormasDeVenta?.Insert(0, defaultFormasDeVenta);
            cbo.DataSource = listaFormasDeVenta;
            cbo.DisplayMember = "FormasDeVenta";
            cbo.ValueMember = "FormasDeVentaId";
            cbo.SelectedIndex = 0;
